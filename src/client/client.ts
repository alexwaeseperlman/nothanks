...
type RoomState = {
  roomId: string;
  state: "lobby" | "inProgress" | "finished";
  players: PlayerView[];
  pot: number;
...
socket.on("stateUpdate", (roomState: RoomState) => {
...
function parseRoomFromLocation(): { roomId: string | null } {
...
function renderGame(): void {
      ...
      <section class="card log-card">
        ...
        <ul class="log">
          ${(room?.events ?? [])
            ...
            .map(
              (entry) =>
                `<li>${formatTimestamp(entry.timestamp)} — ${escapeHtml(entry.message)}</li>`,
            )
  ...
  const passBtn = document.querySelector<HTMLButtonElement>("[data-action='pass']");
  const takeBtn = document.querySelector<HTMLButtonElement>("[data-action='take']");
...
function renderPlayerRow(player: PlayerView): string {
  const badges = [
...
function renderGeneralError(): string {
...
function joinRoom(name: string, options: { suppressError?: boolean } = {}): void {
  ...
  const sendJoin = () => {
    socket.emit("joinRoom", payload, (response: JoinAck | undefined) => {
...
function triggerAction(action: "pass" | "take"): void {
  socket.emit("playerAction", { action });
}

function attachPlayerNameFromState(): void {
